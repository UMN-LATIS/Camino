    ...
    >
      <div class="col px-0">
        <div class="list-group list-group-flush">
          <router-link
            v-for="(stop, index) in fileredTourStops"
            :key="index"
            :to="{ name: 'tour', params: { currentStopId: index } }"
            class="list-group-item list-group-item-action"
            >{{ stop.stop_content.title[$i18n.locale] }}</router-link
          >
        </div>
      </div>
    </b-collapse>
  ...
  computed: {
    ...
    showNextButton: function () {
      ...
      if (
        ...
        this.$store.state.locks[this.currentStopId].filter((e) => e.locked)
          .length > 0
      ) {
        return false;
      }
